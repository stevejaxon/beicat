...
func _ready():
	...
	assert($LeftScreenDetector.connect("body_exited", $Cooper, "playerLeftScreen", [screen_side.LEFT]) == 0)
	assert($RightScreenDetector.connect("body_exited", $Cooper, "playerLeftScreen", [screen_side.RIGHT]) == 0)
